...
const DEFAULTS: FormState = {
  title: '🔥 Бонус для тебя',
  body: 'Зайди в демо — сегодня повышенный кэшбек и новые слоты.',
  url: '/ru',
  icon: '/icon-192.png',
  image: '',
  tag: 'crashgames-broadcast',
}
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 12, padding: '14px 16px' }}>
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 11, color: '#8a6d14', marginBottom: 4, letterSpacing: '0.04em' }}>
            crashgames.demo
          </div>
          <div style={{ fontWeight: 700, color: '#f2e8d0', fontSize: 15, lineHeight: 1.3, marginBottom: 4 }}>
            {form.title || 'Заголовок'}
          </div>
          <div style={{ color: '#a08c5e', fontSize: 13, lineHeight: 1.45 }}>
    ...
    try {
      const res = await fetch('/api/push/send', {
        ...
        body: JSON.stringify({
          ...
          tag: form.tag || 'crashgames-broadcast',
        }),
      })
      ...
      <main style={{ minHeight: '100dvh', display: 'grid', placeItems: 'center', padding: '1.5rem' }}>
        ...
        >
          ...
          <p style={{ color: 'var(--color-text-secondary)', fontSize: '0.875rem', marginBottom: '1.25rem' }}>
            Введите ADMIN_PUSH_SECRET из env Vercel.
          </p>
